# 003_sparse_two_select

模块功能:

用组合特征训练GBDT模型, 使用模型重要性, 筛选出重要性权值不为0的组合特征. 并生成对应这些特征的训练集/验证集/测试集1/测试集2的数据集.

输入:

- data_preprocessing/train_index_2.csv
- 10个训练数据集(part2~part5)的组合特征文件
- 10个验证集数据的组合特征文件
- data_preprocessing/train_part_y.csv   [训练集的label]
- data_preprocessing/evals_y.csv      [验证集的label]

输出:

- data_preprocessing/col_sort_two.csv  [重要性不为0的组合特征存起来]
- 由训练集生成的包含重要性不为0的组合特征的数据集
- 由验证集生成的包含重要性不为0的组合特征的数据集
- 由测试集1生成的包含重要性不为0的组合特征的数据集
- 由测试集2生成的包含重要性不为0的组合特征的数据集 
...
for i in range(1,11): #1~10,10个组合特征文件,每个文件里面有12个特征
...
pd.Series(col).to_csv('data_preprocessing/col_sort_two.csv',index=False) #将重要性不为0的组合特征存起来.
