...
做特征选择, 对稀疏数据按GBDT模型输出的特征重要性, 由特征重要性从高到低进行特征选取.

输入:

- train_index_2.csv
- train_part_x_sparse_one_1.npz
- train_part_x_sparse_one_2.npz
- ...
- train_part_x_sparse_one_8.npz

输出:

- data_preprocessing/col_sort_one.csv       [重要性不为0的特征]
- 最优特征个数

### 特征选择技巧总结:

先用所有特征训练一个GBDT模型, 用来筛选特征. 该模型的auc为baseloss. 记录该模型在验证集上取最优auc时树的棵数为n.

筛选出重要性大于0的特征. 按重要性大小由大到小排序.

以100个特征维度分为一组, 从1~100维特征做为第1组开始组成训练集训练模型, 树的棵数设为n(以便对照), 第2组是1~200维特征...选出auc大于baseloss的模型的训练集中特征个数, 这里选择第1个或2个auc大于baseloss时的模型的特征个数, 作为最优特征个数, 并从稀疏训练集\验证集\测试集中将对应维度的特征数据提取出来.
...
for i in range(1,9):  #各个特征部分, part1~part8
...
pd.Series(col).to_csv('data_preprocessing/col_sort_one.csv',index=False) #输出重要性不为零的特征,从大到小排序
