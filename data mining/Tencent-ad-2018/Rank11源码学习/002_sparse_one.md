...
- train_test_merge.csv

输出数据:

- 第1部分特征:
	- data_preprocessing/train_part_x_sparse_one_1.npz
	- data_preprocessing/evals_x_sparse_one_1.npz
	- data_preprocessing/test1_x_sparse_one_1.npz
	- data_preprocessing/test2_x_sparse_one_1.npz

- 第2部分特征:
	- data_preprocessing/train_part_x_sparse_one_2.npz
	- data_preprocessing/evals_x_sparse_one_2.npz
	- data_preprocessing/test1_x_sparse_one_2.npz
	- data_preprocessing/test2_x_sparse_one_2.npz
- 第3部分特征
- 第4部分特征
- 第5部分特征
- 第6部分特征
...
df = []
print('OneHoting1...')  #由于机器内存问题,编码时分批进行处理
enc = OneHotEncoder()   #单值特征进行onehot编码
...
for feature in one_hot_feature1:
    ...
    arr= []
    del data[feature]  #从data数据表中删除一列,这种操作很方便,比用drop()方便多了.
    print(feature,int(time.time()-s),"s")
print("Saving...")
print('train_part_x...')
...
#interest5中特征值比较多, 比较费时间空间, 所以单独处理
feature = 'interest5'
